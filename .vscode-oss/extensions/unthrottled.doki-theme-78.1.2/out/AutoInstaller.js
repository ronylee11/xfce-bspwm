...
function autoInstallAsset(assetKey, context, assetInstaller) {
    ...
    if (wasTheAssetInstalledYo) {
        const { sticker, } = JSON.parse(context.globalState.get(assetKey));
...
function saveStickerConfig(sticker, context) {
    context.globalState.update(stickerInstallKey, createAssetRestoreConfig(sticker));
}
exports.saveStickerConfig = saveStickerConfig;
function saveWallpaperConfig(sticker, context) {
    context.globalState.update(wallpaperInstallKey, createAssetRestoreConfig(sticker));
}
...
function saveHiddenWatermarkConfig(context) {
    ...
    context.globalState.update(watermarkKey, createAssetRestoreConfig(bestSticker));
}
...
function createAssetRestoreConfig(sticker) {
